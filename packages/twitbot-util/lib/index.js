...
function control(twet) {
	for (var _len = arguments.length, blacklist = Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
		blacklist[_key - 1] = arguments[_key];
	}

	var results = _lodash2.default.flattenDeep(blacklist).map(function (func) {
		return func(twet);
	});
...
function action(twet, args, context) {
	console.log(twet);
	console.log(args);
	...
	console.log(_lodash2.default.flattenDeep(middlewares));
}
