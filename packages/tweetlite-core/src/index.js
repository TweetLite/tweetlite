...
export default class TweetLite {
  ...
  use(methods) {
    if (typeof methods === 'object') {
      ...
      log(`injection extra methods.. ${keys(methods)}`)
    } else {
      const fn = methods
      const name = fn().name
      if (name === '') {
  ...
  extraModules() {
    ...
    this.extra.fullFavoriteList = list => {
      return promiseSeries(
        (id, val) => util.throttle(this.favoriteCreate({ id }), val),
        list
      )
    }
    /**
    ...
    this.extra.fullDestoryFavorite = list => {
      return promiseSeries(
        (id, val) => util.throttle(this.favoriteDestroy({ id }), val),
        list
      )
    }
    /**
    ...
    this.extra.fullUserFollow = list => {
      return promiseSeries(
        (user_id, val) => util.throttle(this.userCreate({ user_id }), val),
        list
      )
    }
    /**
    ...
    this.extra.fullUserDestroy = list => {
      return promiseSeries(
        (user_id, val) => util.throttle(this.userDestroy({ user_id }), val),
        list
      )
    }
    /**
    ...
    this.extra.fullUserMessage = (list, msg) => {
      return promiseSeries(
        (user_id, val) =>
          util.throttle(this.messageCreate({ user_id, text: msg }), val),
        list
      )
    }
    /**
     ...
     * @api public
     */
    this.extra.fullFollowers = async (
      opt = { count: 5000, stringify_ids: true }
    ) => {
      try {
     ...
     * @api public
     */
    this.extra.fullFollowings = async (
      opt = { count: 5000, stringify_ids: true }
    ) => {
      try {
    ...
    this.extra.fullFavorites = async opt => {
      try {
        ...
        const favorites = await favorite(
          Object.assign({ count: 200, include_entities: false }, opt),
          'favorites'
        )
        return favorites
      } catch (err) {
    ...
    log(`injection extra functions.. ${keys(this.extra)}`)
