...
TT.prototype.StreamAction = function(twet,name,notification,favorite){
	var dump = [];
	var msg = " takip ettik";

	var confd =conf.get(name);
	
	if(!_.contains(confd.blacklist, twet.user.id) && twet.lang == confd.lang){
				...
				.then(function(data){
						...
						if(notification && notification != "null" && notification != null){
							if(os.platform() =='linux'){
								libnotify.notify(msg, { title: util.Hal(twet.user.screen_name,'i') ,image:'twitbot'});
