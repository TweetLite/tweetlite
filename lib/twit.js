...
TT.prototype.StreamAction = (twet, notification, favorite, takip) => {
	const dump = [];
	let msg = null;
			...
			if (data[0] !== null && !_.has(data[0], 'errors')) {
				...
				if (notification && notification !== null) {
					if (os.platform() === 'linux') {
						...
						trayballoon({text: `${util.hal(twet.user.screen_name, 'i')} ${message}`, icon: '../lib/assets/48.ico', timeout: 20000});
					}
				}
			}
		});
...
TT.prototype.FullUserDestroy = list => {
...
TT.prototype.FullUserFollow = list => {
