...
TT.prototype.StreamAction = function(twet,notification,favorite,takip){
	...
	Promise.all([that.Blocks(),that.Friends()])
	.then(function(data){

			if(!_.contains(data[0].ids, twet.user.id) &&  !_.contains(data[1].ids, twet.user.id)){
				
				if(takip =="Evet"){
					dump.push(that.UserCreate({user_id:twet.user.id_str,follow:true}));
				} else if(favorite == "Evet"){
					dump.push(that.Favorite({id:twet.id_str}));
				}

				if(takip =="Evet" && favorite == "Evet" ){
