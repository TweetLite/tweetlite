			...
			inquirer.prompt(ques, answers => {
				...
				T.userLookup({screen_name: SearchName}).then(data => {
					...
					if (respons === null) {
						...
						console.log(`  ${clor.green(`${util.haller(respons.screen_name, 'i')} added to the banned list ..`)}`);
						return T.blocks(); // wtf this
					}
				}).catch(err => {
				  console.error(err);
				});
	...
	case 'F' :
		if (_.allKeys(nconf.get('users')).length > 0) {
			if (args.stop) {
				...
				forever.startDaemon(path.join(__dirname, './dist/worker.js'), {
					uid: `twitbot_${name}`,
					args:
		   ...
		   typeof process.env.twitbotKeywords == 'undefined') {
			   ...
			   if (args.stop) {
				   ...
				   forever.list(false, function (err, __process) {
					   ...
					   const twitbot = util.ForeverCheck(__process);
				 ...
				 forever.startDaemon(path.join(__dirname, '../dist/worker.js'), {uid: `twitbot_${process.env.twitbotUsername}`});
			   }
           }
		break;
	case 'version':
