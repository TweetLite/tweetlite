...
switch (cmd[2]) {
	...
	case 'S':
		if (util.checkUser()) {
			util.prompt('start').then(answers => {
				...
				return Promise.all([T.searchFollow({q:keyword,takip_sayi:takip_sayi, lang:confd.lang}), answers,T]);
		  	}).then(result => {
				const T = result[3];
				const infoList = [];
	...
	(async()=>{
		if (util.checkUser()) {
			try{
				...
				const confd = util.getUser(select_account);
				const T = new TT(confd);
				const msg = util.spinnerMsg(`Twitbot searching for `);
				const favoriteList = await T.favorites();
				const favoriteDestory = await T.FullDestoryFavorite(favoriteList);
				clearInterval(msg);
	...
	case 'FL' :
		(async()=>{
			if (util.checkUser()) {
				...
				let {select_account} = await util.prompt('accountlist', _.keys(users));
	...
	case 'M':
		if (util.checkUser()) {
			util.prompt('messagelist').then((answers) => {
				const confd = util.getUser(answers.select_account);
				const T = new TT(confd);

				if(answers.send === 'Yes' && answers.message) {
					return Promise.all([T,T.fullFollowerList(),answers])
				}
				return null;
			}).then( result => {
				...
				if( result === null) {
					...
					const Tx = result[0];
					let list = result[1];
					const answers = result[2];

					list = list.ids.slice(0, answers.limit);
					Tx.fullUserMessage(list, answers.message).then(respons => {
	...
	case 'U' :
		if (util.checkUser()) {
			util.prompt('unfollowlist').then((answers) => {
				const confd = util.getUser(answers.select_account);
				const T = new TT(confd);
