...
import _ from 'underscore'; // lodash ile değiştirilmesi gerek
...
switch (cmd[2]) {
		...
		if (_.allKeys(nconf.get('users')).length > 0) {
			...
			inquirer.prompt(ques, answers => {
				...
				.then(data => {
					...
					// düzgün çalışmıyor SearchUsersList kullanılmamış buraada sorun var tekrar yazılması gerek
	...
	case 'FL' :
		...
		ques[0].choices = _.allKeys(users);
		inquirer.prompt(ques, answers => {
	...
	case 'U' :
		if (_.allKeys(nconf.get('users')).length > 0) {
			...
			inquirer.prompt(ques, answers => {
				...
				// true hepsini sil
				// false sadece takip etmeyenleri sil anlamına geliyor
				// bütün takipçiler karşılaştırılmıyor bütün listeyi sorgulayan hale geçirilmeli
				// karşılaştırma düzgün çalışmıyor olabilir
				T.notFollowingList(false).then(data => {
